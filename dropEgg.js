Game.DropEgg=function(failRate)
{
	...
	if (random>=failRate)
	{
		...
		{
			Game.Notify('Almost Got One!','You already have a <b>'+drop+'</b>',Game.Upgrades[drop].icon)
			if (Math.random()<0.1) drop=choose(Game.rareEggDrops);
			else drop=choose(Game.eggDrops);
	...
	} else Game.Notify('No Egg Drop For You!',Beautify(random*100)+'/'+Beautify(failRate*100),[0,12]);
};
